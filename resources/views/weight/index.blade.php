@extends('layouts.app')

@section('content')
<div class="p-3 mb-2 bg-warning text-dark">
    <div class="container">
    	<div class="form-group">
    	    <div class="row justify-content-center mb-2">
	            <h1>My Workout</h1>
	        </div>
    	    <form>
                <div class="row mb-3">
                    <label for="weight" class="col-sm-2 col-form-label">体重(kg)</label>
                    <div class="col-sm-10">
                        <input type="number" step="0.1" class="form-control" id="weight" name="weight">
                    </div>
                </div>
                <div class="row mb-3">
                    <label for="fatpercentage" class="col-sm-2 col-form-label">体脂肪率(％)</label>
                    <div class="col-sm-10">
                        <input type="number" step="0.1" class="form-control" id="fatpercentage" name="fatpercentage">
                    </div>
                </div>
                <div class="row mb-3">
                    <label for="fatmass" class="col-sm-2 col-form-label">体脂肪量(kg)</label>
                    <div class="col-sm-10">
                        <input type="fatmass" step="0.1" class="form-control" id="fatmass" name="fatmass">
                    </div>
                </div>
                <div class="row mb-3">
                    <label for="musclemass" class="col-sm-2 col-form-label">筋肉量(kg)</label>
                    <div class="col-sm-10">
                        <input type="number" step="0.1" class="form-control" id="musclemass" name="musclemass">
                    </div>
                </div>
                <div class="row mb-3">
                    <label for="leanbodymass" class="col-sm-2 col-form-label">除脂肪体重(kg)</label>
                    <div class="col-sm-10">
                        <input type="number" step="0.1" class="form-control" id="leanbodymass" name="leanbodymass">
                    </div>
                </div>
                <div class="row mb-3">
                    <label for="bmi" class="col-sm-2 col-form-label">BMI</label>
                    <div class="col-sm-10">
                        <input type="number" step="0.1" class="form-control" id="bmi" name="bmi">
                    </div>
                </div>
                <div class="row mb-3">
                    <label for="west" class="col-sm-2 col-form-label">ウエスト(cm)</label>
                    <div class="col-sm-10">
                        <input type="number" step="0.1" class="form-control" id="west" name="west">
                    </div>
                </div>
            </form>
        </div>
	</div>
</div>
@endsection